...
---
## Prepare
### The four main data files are loaded in RStudio
```{r}
...
glimpse(daily_activity)
```
### A detailed summary is seen for each data set
### Now date and time coloums in each dataset is formatted in a required way making it easy for analysis
```{r}
...
```
## The sleep_activity is joined in SQL and thus imported
```{r}
setwd("/Users/deyah/Desktop/Case Study/New Data")
...
```
## We observe that on Monday with more steps, the sleep time is declining
```{r}
...
```
### The above pie gives us an idea that most people have sedentary active minutes
```{r}
ggplot(data=daily_activity,aes(x=TotalSteps,y=Calories))+geom_point()+stat_smooth(method=lm) +labs(title="Total Steps taken Vs Total Calorie Burnt")
```
